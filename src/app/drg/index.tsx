...
export default function Page() {
  ...
  const setUploadConfig = async () => {
    const res = await ReportService.getUploadConfig();
    if (res.statusCode === 200) {
      return res.data;
    } else {
      console.error('Failed to fetch upload config');
    }
  };
  ...
  useEffect(() => {
    setUploadConfig().then((res) => {
      const fileListParams = {
        bucket: '',
        maxResults: 200,
        prefix: `${res.targetLocation}DRG/Thumbs/`,
        delimiter: '/',
        sharedFlag: false,
